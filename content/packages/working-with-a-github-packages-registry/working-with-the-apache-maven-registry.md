...
The above approach is simplified to an unhelpful degree for the purposes of explanation. You shouldn't store your token in plaintext in settings.xml.
A better approach would be to store the token in a GitHub secret, write that secret to an environment variable, and have Maven reference it.
1. Create a GitHub secret and store your PAT in it. This example uses a "secret" named `your_token_name`.
2. Pick an environment variable name. This example uses `gh_packages_token`.
3. In your GitHub workflow where you run Maven, set the environment variable to your PAT using your secret:
  ```yml
  - name: ☕ Run Maven and interact with the GitHub package registry
    run: mvn deploy -s settings.xml
    env:
      gh_packages_user: ${{ github.actor }}
      gh_packages_token: ${{ secrets.your_token_name }}

  ```
4. Use the following syntax in settings.xml to tell Maven to read these values from environment variables:
  ```xml
    <servers>
      <server>
        <id>github</id>
        <username>${env.gh_packages_user}</username>
...
Note that you can't use a token generated by a GitHub App because they aren't valid for the package registry. The interface for choosing scopes for your GitHub App lets you assign the `read:packages` scope, but I can't tell why. Have fun either letting your production build rely on a particular user's PAT (which will stop working suddenly if they leave) or paying for an extra 'seat' license in perpetuity for the sole purpose of making tokens for jobs like these. I was pretty sure that the point of GitHub Apps was to get around that, but I guess not.
