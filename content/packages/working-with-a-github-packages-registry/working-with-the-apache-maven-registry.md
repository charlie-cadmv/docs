...
A better approach would be to store the token in a GitHub secret (`your_token_name` in this example), write that secret to an environment variable (`gh_packages_token`), and have Maven reference it like this:
```xml
...
  <servers>
    <server>
...
```
Then in your workflow where you run Maven, set the gh_packages_token environment variable to your PAT using your secret:
```yml
- name: ☕ Run Maven and interact with the GitHub package registry
  run: mvn <your args here>
  env:
